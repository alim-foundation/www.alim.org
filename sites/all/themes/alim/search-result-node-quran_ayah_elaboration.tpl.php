...
foreach ($tax as $singletax){
	if($singletax->vid == 1){ $translator = $singletax->name; }
	if($singletax->vid == 3){ $suraname = $singletax->name; }
}
$titletxt = 'Surah '.$node_result->field_surah_no[0]['value'].' - '.$suraname.' , Ayah '.$node_result->field_ayah_no[0]['value'] .' Commentary - '.$node_result->field_note_id[0]['value'];
?>
<dt class="title">
<?php   // Print title of search result
$urlto = 'library/quran/surah/english/'.$node_result->field_surah_no[0]['value'].'/'.$node_result->field_quran_bk_code[0]['value'] ;
$hash = "note-".$node_result->field_note_id[0]['value'];
print l($titletxt,$urlto,array('fragment' => $hash, 'html' => TRUE ));
?>
</dt>
<dd>
  <?php if ($snippet) : ?>
