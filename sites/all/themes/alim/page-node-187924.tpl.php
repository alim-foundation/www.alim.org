...
$query1=db_query('SELECT  users.uid, `name`  ,mail as email ,CAST(profile_values.value AS CHAR) AS fname FROM `users`,profile_values WHERE  '.$qry_string.' AND profile_values.fid=18 AND profile_values.uid = users.uid  GROUP BY uid DESC');
